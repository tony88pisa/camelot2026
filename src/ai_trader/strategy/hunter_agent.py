...
class HunterAgent:
    ...
    def identify_opportunities(self, top_n: int = 20, budget: float = None) -> List[Dict]:
        ...
        # v10.45: Recupero budget reale tramite summary normalizzato
        if budget is None:
            summary = self.adapter.get_account_summary()
            budget = summary.get("free_quote_balance", 0.0) if summary.get("ok") else 0.0
            
        current_budget = budget * 0.95
        ...
        for cand in top_candidates:
            ...
            min_notional = 1.0 # Standard 2026 per coppie EUR (PEPE inclusive)
            if symbol_info:
                notional_filter = next((f for f in symbol_info.get('filters', []) if f['filterType'] in ['NOTIONAL', 'MIN_NOTIONAL']), None)
                if notional_filter:
                    min_notional = float(notional_filter.get('minNotional', 1.0))
