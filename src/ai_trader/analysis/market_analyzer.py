...
class MarketAnalysis:
    ...
    recommended_tp_pct: float # v10.50: Profit target consigliato (es. 0.5 per 0.5%)
...
class MarketAnalyzer:
    ...
    def analyze(self, symbol: str, klines: list[list] | None = None,
        ...
        try:
            ...
            # --- QUANTUM ALPHA v10.50: DYNAMIC TP CALCULATION ---
            # Base profit: 0.4%
            # Bonus basato su trend (max +1.5%) e volume (max +0.6%)
            tp_bonus_trend = max(0.0, trend_score * 1.5) if regime == "bull" else 0.0
            tp_bonus_vol = min(0.6, (volume_ratio - 1.0) * 0.2) if volume_ratio > 1.0 else 0.0
            recommended_tp = 0.4 + tp_bonus_trend + tp_bonus_vol
            ...
            logger.debug(f"Analisi Apex {symbol}: RSI={result.rsi}, Trend={result.trend_score}")
            return result
    ...
    def investigate_deeply(self, symbol: str) -> dict[str, Any]:
        ...
        ctx = self.get_mtf_context(symbol)
        if not ctx["ok"]:
            return {"survival_score": 50, "reason": ctx.get("error", "MTF Error"), "ok": False}
        ...
        return {
            ...
            "reason": reason,
            "ok": True,
            "mtf_summary": ctx["summary"]
        }

    def get_mtf_context(self, symbol: str) -> dict[str, Any]:
        """
        Raccoglie dati tecnici su pi timeframe (4h, 1h, 15m) per il MAD Cycle v10.4.
        """
        if not self.adapter:
        ...
        for timeframe in intervals:
            klines = self.adapter.get_klines(symbol, interval=timeframe, limit=30)
            if not klines or len(klines) < 21:
                results[timeframe] = "Dati insufficienti"
            ...
            results[timeframe] = f"Price: {closes[-1]} | RSI: {round(rsi, 2)} | Trend: {trend}"
            
            # Punteggio 0-100: Trend (60%) + RSI (40%)
            trend_val = 1.0 if trend == "BULL" else -1.0
            rsi_val = (rsi - 50) / 50
            scores[timeframe] = (trend_val * 0.6 + rsi_val * 0.4) * 50 + 50

        summary = (
            f"--- MTF CONTEXT for {symbol} ---\n"
            f"MACRO (4h): {results['4h']}\n"
            f"MESO (1h): {results['1h']}\n"
            f"MICRO (15m): {results['15m']}\n"
            f"CONFLUENZA: {'ALLINEATA' if scores['4h'] > 60 and scores['15m'] > 60 else 'DIVERGENTE'}"
        )
