...
class GridEngine:
    ...
    def evaluate(self, symbol: str, current_price: float) -> list[dict[str, Any]]:
        """
        Valuta la griglia dato il prezzo corrente.
        Restituisce una lista di azioni da eseguire (BUY/SELL).
        ...
        Returns:
            Lista di azioni: [{"action": "BUY"/"SELL", "level": int, "price": float, "usdt": float}, ...]
        """
        if symbol not in self.grids:
            return []
        ...
        for level in grid.levels:
            ...
            if level.status == "empty" and current_price <= level.price:
                ...
                if order_usdt >= 5.0:  # Min Binance order
                    actions.append({
                        "action": "BUY",
                        "symbol": symbol,
            ...
            # SELL: prezzo corrente  salito sopra il livello successivo e il livello  comprato
            elif level.status == "bought" and current_price >= level.price:
                # Vendi solo se il prezzo  sopra il prezzo di acquisto + step minimo
                step = (grid.config.upper_price - grid.config.lower_price) / grid.config.num_levels
                min_sell_price = level.buy_price * 1.003  # almeno +0.3% di profitto (copre fee)
