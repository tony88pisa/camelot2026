#!/usr/bin/env python3
# main.py
# 2026-04-13 - QUANTUM HUNTER v10.0 (MAD NEURAL SINGULARITY)
"""
...
class GuardianThread(threading.Thread):
    """
    Sotto-processo dedicato alla sicurezza (v10.3).
    Corre al bot e controlla la salute del portafoglio ogni 10 secondi.
    """
    def __init__(self, paladin: PaladinAgent, interval: int = 10):
    ...
    def run(self):
        logger.info(f"Guardian: Avviato monitoraggio costante (Intervallo: {self.interval}s)")
        while not self.stop_event.is_set():
            try:
                health = self.paladin.check_portfolio_health()
                if health["status"] == "EMERGENCY":
                    logger.critical("!!! GUARDIAN: EMERGENZA RILEVATA! Stop immediato !!!")
                    # Il Paladin ha gi liquidato e creato il file di lock. 
                    # Segnaliamo al processo principale di fermarsi.
                    global _running
                    _running = False
                    break
            except Exception as e:
                logger.error(f"Guardian: Errore durante audit sicurezza: {e}")
...
def main():
    ...
    # --- IGNITION: GUARDIAN THREAD v10.3 ---
    guardian = GuardianThread(paladin=paladin, interval=10)
    guardian.start()
    ...
    while _running:
        ...
        try:
            ...
            logger.info(f"v10.0 'MAD Neural' Cycle #{cycle_count} START")
            ...
            if cycle_count % 3 == 0:
                logger.info("AI DREAM (MAD): Inizio ciclo di riflessione multi-agente...")
                dream_agent.run_dream_cycle()
                
                # --- AUTO-EVOLUZIONE CODICE v10.0 ---
                logger.info("EVOLUTION: Controllo lezioni certificate per auto-ottimizzazione...")
        ...
        # --- SICUREZZA POST-CICLO ---
        if not _running or Path("PALADIN_LOCK.txt").exists():
            logger.critical("Main: Rilevato blocco di sicurezza o emergenza Guardian. Spegnimento.")
            break

        time.sleep(interval)

    # Shutdown ordinato
    logger.info("Main: Spegnimento del sistema in corso...")
    guardian.stop()
